...
class TransactionLog(AVL):
    ...
    def _update(self):
        ...
        if self.left and self.right:
            self.sum = self.item.d + self.left.sum + self.right.sum
        elif self.left:
            self.sum = self.item.d + self.left.sum
        elif self.right:
            self.sum = self.item.d + self.right.sum
        else:
            self.sum = self.item.d

        self.min = self._min_node()
        self.max = self._max_node()
    ...
    def interval_revenue(self, t1, t2):
        ...
        if self.min.item.key >= t1 and self.max.item.key <= t2:
            return self.sum
        elif self.min.item.key >= t1 or self.max.item.key <= t2 or (self.min.item.key <= t1 and self.max.item.key >= t2):
            if t1 < self.item.key < t2:
                if self.left and self.right:
                    return self.left.interval_revenue(t1, t2) + self.right.interval_revenue(t1, t2) + self.item.d
                elif self.left:
                    return self.left.interval_revenue(t1, t2) + self.item.d
                elif self.right:
                    return self.right.interval_revenue(t1, t2) + self.item.d
                return self.item.d
            elif self.item.key < t1:
                if self.right:
                    return self.right.interval_revenue(t1, t2)
                return 0
            elif self.item.key > t2:
                if self.left:
                    return self.left.interval_revenue(t1, t2)
                return 0

            elif self.item.key == t1:
                if self.left and self.left.max.item.key == t1:
                    if self.right:
                        return self.left.interval_revenue(t1, t2) + self.right.interval_revenue(t1, t2) + self.item.d
                    return self.left.interval_revenue(t1, t2) + self.item.d
                if self.right:
                ...
                if self.right and self.right.min.item.key == t2:
                    if self.left:
                        return self.right.interval_revenue(t1, t2) + self.left.interval_revenue(t1, t2) + self.item.d
                    return self.right.interval_revenue(t1,t2) + self.item.d
                if self.left:
                    return self.left.interval_revenue(t1, t2) + self.item.d
                return self.item.d
